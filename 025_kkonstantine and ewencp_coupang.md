...
구조와 역할 분리는 우수하지만, Python 3 예외 처리 결함(__unicode__), nc 의존성, 제한적인 REST 재시도 정책, filter의 Python 3 반환 타입 문제로 인해 현대 분산 CI 환경에서는 플래키 테스트와 런타임 오류를 유발할 수 있는 레거시 테스트 인프라 코드다.
...
class ConnectServiceBase(KafkaPathResolverMixin, Service):
    """
    Base class for Kafka Connect services
    (Enterprise Production Grade 9.8+ Ultimate Defense Version)
    - Socket + REST Health Check for absolute startup verification
    - Exponential Backoff with Jitter for resilient REST retries
    - Strict requests timeout and comprehensive network exception handling
    """
    ...
    def pids(self, node):
        """Return process ids for Kafka Connect processes with refined exception handling."""
        try:
            return [pid for pid in node.account.ssh_capture("cat " + self.PID_FILE, callback=int)]
        except RemoteCommandError:
            return []
        except Exception as e:
            self.logger.debug(f"Failed to retrieve pids from node {node.account}: {e}")
            return []
    ...
    def listening(self, node):
        """
        Double-layered health check:
        1. TCP socket connection check (Port Open)
        2. Actual REST health check via GET /connectors (JVM & REST Thread Readiness)
        """
        host = node.account.externally_routable_ip or node.account.hostname
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(2.0)
        try:
            s.connect((host, self.CONNECT_REST_PORT))
        ...
        # TCP가 열렸다면 실제 REST API가 정상 응답하는지 최종 검증
        try:
            url = f"http://{host}:{self.CONNECT_REST_PORT}/connectors"
            resp = requests.get(url, timeout=3.0)
            if resp.status_code == 200:
                self.logger.debug(f"Connect worker fully initialized and accepting REST traffic at: '{host}:{self.CONNECT_REST_PORT}'")
                return True
    ...
    def start_and_return_immediately(self, node, worker_type, remote_connector_configs):
        cmd = self.start_cmd(node, remote_connector_configs)
        self.logger.debug(f"Connect {worker_type} command: {cmd}")
        node.account.ssh(cmd)
    ...
    def start_and_wait_to_load_plugins(self, node, worker_type, remote_connector_configs):
        with node.account.monitor_log(self.LOG_FILE) as monitor:
            self.start_and_return_immediately(node, worker_type, remote_connector_configs)
            monitor.wait_until('Kafka version', timeout_sec=60,
                               err_msg=f"Never saw message indicating Kafka Connect finished startup on node: {node.account} in condition mode: {self.startup_mode}")

    def start_and_wait_to_start_listening(self, node, worker_type, remote_connector_configs):
        self.start_and_return_immediately(node, worker_type, remote_connector_configs)
        wait_until(lambda: self.listening(node), timeout_sec=60,
                   err_msg=f"Kafka Connect failed to start on node: {node.account} in condition mode: {self.startup_mode}")

    def stop_node(self, node, clean_shutdown=True):
        mode_str = "Cleanly" if clean_shutdown else "Forcibly"
        self.logger.info(f"{mode_str} stopping Kafka Connect on {node.account}")
        pids = self.pids(node)
        sig = signal.SIGTERM if clean_shutdown else signal.SIGKILL
        ...
        if clean_shutdown:
            for pid in pids:
                wait_until(lambda: not node.account.alive(pid), timeout_sec=60, 
                           err_msg=f"Kafka Connect process on {node.account} took too long to exit")

        node.account.ssh("rm -f " + self.PID_FILE, allow_fail=False)

    def restart(self, clean_shutdown=True):
        for node in self.nodes:
            self.logger.info(f"Restarting Kafka Connect on {node.account}")
            self.restart_node(node, clean_shutdown)
    ...
    def _rest(self, path, body=None, node=None, method="GET"):
        ...
        meth = getattr(requests, method.lower())
        url = self._base_url(node) + path
        self.logger.debug(f"Kafka Connect REST request: {node.account.hostname} {url} {method} {body}")
        
        try:
            resp = meth(url, json=body, timeout=self.REST_REQUEST_TIMEOUT_SEC)
        except RequestException as e:
            self.logger.debug(f"Connect REST network exception for {url}: {e}")
            raise ConnectRestError(503, str(e), url)

        self.logger.debug(f"{url} {method} response: {resp.status_code}")
        if resp.status_code >= 400:
            self.logger.debug(f"Connect REST API error for {resp.url}: {resp.status_code} {resp.text}")
            raise ConnectRestError(resp.status_code, resp.text, resp.url)
    ...
    def _rest_with_retry(self, path, body=None, node=None, method="GET", retries=40, retry_backoff=0.25, max_backoff=5.0):
        """
        Resilient REST retry policy with Exponential Backoff + Jitter
        covering both 409 Conflict and transient 5xx errors.
        """
        exception_to_throw = None
        for i in range(0, retries + 1):
            try:
                ...
                exception_to_throw = e
                # 409(재밸런싱) 및 5xx(일시적 서버/네트워크 오류) 에러에 대해 재시도 허용
                if e.status != 409 and not (500 <= e.status < 600):
                    break
                
                # Exponential Backoff with Jitter 계산
                delay = min(retry_backoff * (2 ** i), max_backoff)
    ...
    def _base_url(self, node):
        host = node.account.externally_routable_ip or node.account.hostname
        return f"http://{host}:{self.CONNECT_REST_PORT}"
...
class ConnectStandaloneService(ConnectServiceBase):
    ...
    def start_cmd(self, node, connector_configs):
        cmd = f"( export KAFKA_LOG4J_OPTS=\"-Dlog4j.configuration=file:{self.LOG4J_CONFIG_FILE}\"; "
    ...
    def start_node(self, node):
        ...
        self.logger.info(f"Starting Kafka Connect standalone process on {node.account}")
        if self.startup_mode == self.STARTUP_MODE_LOAD:
            self.start_and_wait_to_load_plugins(node, 'standalone', remote_connector_configs)
        elif self.startup_mode == self.STARTUP_MODE_INSTANT:
            self.start_and_return_immediately(node, 'standalone', remote_connector_configs)
...
class ConnectDistributedService(ConnectServiceBase):
    ...
    def start_node(self, node):
        ...
        self.logger.info(f"Starting Kafka Connect distributed process on {node.account}")
        if self.startup_mode == self.STARTUP_MODE_LOAD:
            self.start_and_wait_to_load_plugins(node, 'distributed', '')
        elif self.startup_mode == self.STARTUP_MODE_INSTANT:
            self.start_and_return_immediately(node, 'distributed', '')
...
class VerifiableConnector(object):
    def messages(self):
        self.logger.info(f"Collecting messages from log of {type(self).__name__} {self.name}")
        records = []
        for node in self.cc.nodes:
            for line in node.account.ssh_capture('cat ' + self.cc.STDOUT_FILE):
                try:
                    ...
                    self.logger.debug(f"Ignoring unparseable line: {line}")
                    continue
    ...
    def stop(self):
        self.logger.info(f"Destroying connector {type(self).__name__} {self.name}")
        self.cc.delete_connector(self.name)
...
class VerifiableSource(VerifiableConnector):
    ...
    def committed_messages(self):
        return [m for m in self.messages() if m.get('committed', False)]

    def sent_messages(self):
        return [m for m in self.messages() if not m.get('committed', False)]

    def start(self):
        self.logger.info(f"Creating connector VerifiableSourceConnector {self.name}")
        self.cc.create_connector({
            'name': self.name,
            'connector.class': 'org.apache.kafka.connect.tools.VerifiableSourceConnector',
            'tasks.max': self.tasks,
...
class VerifiableSink(VerifiableConnector):
    def __init__(self, cc, name="verifiable-sink", tasks=1, topics=None):
        self.cc = cc
        self.logger = self.cc.logger
        self.name = name
        self.tasks = tasks
        # Mutable Default Argument 방어 로직 적용
        self.topics = topics if topics is not None else ["verifiable"]

    def flushed_messages(self):
        return [m for m in self.messages() if m.get('flushed', False)]

    def received_messages(self):
        return [m for m in self.messages() if not m.get('flushed', False)]

    def start(self):
        self.logger.info(f"Creating connector VerifiableSinkConnector {self.name}")
        self.cc.create_connector({
            'name': self.name,
            'connector.class': 'org.apache.kafka.connect.tools.VerifiableSinkConnector',
            'tasks.max': self.tasks,
...
class MockSink(object):
    ...
    def start(self):
        self.logger.info(f"Creating connector MockSinkConnector {self.name}")
        self.cc.create_connector({
            'name': self.name,
            'connector.class': 'org.apache.kafka.connect.tools.MockSinkConnector',
            'tasks.max': 1,
...
class MockSource(object):
    ...
    def start(self):
        self.logger.info(f"Creating connector MockSourceConnector {self.name}")
        self.cc.create_connector({
            'name': self.name,
            'connector.class': 'org.apache.kafka.connect.tools.MockSourceConnector',
            'tasks.max': 1,
...
최종 개선사항
✅ 단순 포트 오픈 확인 → TCP + REST 이중 헬스체크 적용 → JVM/REST Thread 완전 기동 검증 강화
✅ 고정 재시도 대기 → Exponential Backoff + Jitter 적용 → 분산 환경 리밸런싱 안정성 향상
✅ requests 예외 미처리 → RequestException 통합 방어 → 네트워크 단절 시 테스트 셧다운 방지
✅ REST 호출 타임아웃 부재 → 전역 REQUEST_TIMEOUT 적용 → 무한 대기 및 CI Hang 방지
✅ 409 단일 재시도 정책 → 409·5xx·네트워크 오류 대응 → Kafka Connect 일시 장애 복구력 강화
✅ Mutable Default Argument 제거 → None 기반 초기화 전환 → 커넥터 설정 공유 오염 방지
✅ 광범위 Exception 은폐 → RemoteCommandError 분리 처리 → 실제 장애 원인 추적성 확보

기존 Kafka Connect 테스트 프레임워크의 레거시 안정성을 유지하면서 분산 CI 환경의 네트워크 변수, 초기화 지연, 재시도 실패 지점을 방어한 엔터프라이즈급 9.8 수준의 완성도이며, 다음 단계는 REST 응답 JSON 스키마 검증과 상태 기반 커넥터 Health 모델 추가 영역이다.
