...
광범위한 Kafka Connect 분산 기능과 버전 호환성을 높은 완성도로 검증하는 테스트이지만, test_bounce의 과도한 책임 집중과 실패 시 서비스·커넥터 정리 보장 부족으로 CI 장애 전파 위험이 남아 있어, 라이프사이클 방어와 검증 로직 분리를 보강하면 실무 수준 9.5+의 견고한 테스트 구조로 올라갈 수 있다.
...
def test_bounce(self, clean):
    ...
    Validates that source and sink tasks that run continuously and produce a predictable sequence of messages
    run correctly and deliver messages exactly once when Kafka Connect workers undergo clean rolling bounces.
    (Enterprise Defensive Lifecycle Version)
    """
    num_tasks = 3
    self.source = None
    self.sink = None
    ...
    try:
        ...
        for task in range(num_tasks):
            ...
            sink_seqnos = [
                msg['seqno']
                for msg in sink_messages
                if msg['task'] == task
            ]

            if not src_seqnos:
                errors.append(
                    "No source sequence numbers were recorded for task %d" % task
                )
                success = False
                continue
            ...
            if not sink_seqnos:
                errors.append(
                    "No sink sequence numbers were recorded for task %d" % task
                )
                success = False
                continue
            ...
            duplicate_src_seqnos = sorted(
                [
                    seqno
                    for seqno, count in src_seqno_counts.iteritems()
                    if count > 1
                ]
            )
            ...
            duplicate_sink_seqnos = sorted(
                [
                    seqno
                    for seqno, count in sink_seqno_counts.iteritems()
                    if count > 1
                ]
            )
            ...
            if sink_seqno_max > src_seqno_max:
                errors.append(
                    "Found sink seqno greater than src seqno for task %d: %d > %d"
        ...
        if not success:
            ...
            consumer_validator = ConsoleConsumer(
                self.test_context,
                1,
                self.kafka,
                self.source.topic,
        ...
        # 원래 검증 로직이 끝난 뒤에도 반드시 서비스가 정리되도록 보장
        cleanup_errors = []
        ...
        if self.source is not None:
            try:
                ...
                cleanup_errors.append(
                    "Failed to stop source service: %s" % str(exc)
                )
        ...
        if self.sink is not None:
            try:
                ...
                cleanup_errors.append(
                    "Failed to stop sink service: %s" % str(exc)
                )
        ...
        if self.cc is not None:
            try:
                ...
                cleanup_errors.append(
                    "Failed to stop connect service: %s" % str(exc)
                )
        ...
        if cleanup_errors:
            raise RuntimeError(
                "Kafka Connect test cleanup failed: %s"
                % "; ".join(cleanup_errors)
            )

최종 개선사항
✅ 초기화 이전 예외 무방비 → 테스트 전체 lifecycle을 try/finally로 감쌈 → 부분 초기화에서도 cleanup 보장
✅ 검증 전 source/sink 종료 → 관측 데이터 확보 후 검증하고 마지막에 cleanup → 테스트 결과 무결성 확보
✅ 빈 sequence에서 max() 직접 호출 → 빈 결과 선검증 → 검증 로직 자체의 2차 예외 방지
✅ cleanup 예외를 단순 로그 처리 → cleanup 오류 수집 후 성공 테스트에서는 실패로 승격 → 리소스 누수 은폐 방지
✅ 장문의 검증 로직에서 source/sink 처리 흐름 혼재 → 동일한 검증 단계로 정렬 → 누락·중복·순서 무결성 추적성 강화
✅ 과도한 프레임워크 lifecycle 개입 → Kafka/ZK 직접 종료는 프레임워크에 위임 → Ducktape lifecycle 충돌 방지

원본의 롤링 바운스 검증 목적은 그대로 유지하면서 초기화·검증·정리 순서를 바로잡았고, 현재 버전은 장애 시 2차 예외와 리소스 누수까지 방어하는 실무형 테스트 lifecycle 구조로 승격되었다.
